...
const tokenizer = (code) => {
    ...
    while (pos < code.length) {
        ...
        if (counter > code.length) {
            throw new Error('Stuck tokenizing.');
        }
    }
    return tokens;
};
...
const nextIs = (state, type) => {
    if (state.pos < state.tokens.length && state.tokens[state.pos].type === type) {
        return true;
    }
...
const expectValue = (state) => {
    ...
    if (state.tokens[state.pos].type !== NULL && state.tokens[state.pos].type !== BOOLEAN && state.tokens[state.pos].type !== NUMBER && state.tokens[state.pos].type !== STRING) {
...
const parser = (tokens) => {
    const state = {
        pos: 0,
        tokens: tokens.filter(token => token.type !== LINEFEED).filter(token => token.type !== CARRIAGERETURN),
    };
    const object = {};
    let continueLoop = true;
