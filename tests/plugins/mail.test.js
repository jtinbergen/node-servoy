...
test('sendMail fails without configured transport', async () => {
    let rejected = false;
    try {
        mail.sendMail('test@demo.com', 'from@demo.com', 'Test subject', 'Expired.').catch(e => rejected = true);
    } catch (e) {
...
test('sendMail uses configured transport', async () => {
    let options = {};
    mail.configureMailTransport((to, from, subject, msgText, cc, bcc, attachments) => {
        options.from = from;
        options.to = to;
